...
<html lang="en">
  ...
  <section id="container" class="">
              ...
              <div class="row">
                  <div class="col-lg-12">
                      <section class="panel">
                          ...
                          <strong>项目基本信息列表</strong>
                                                       
                   <input type="button" value="导出Excel" class="btn btn-default pull-right btn-sm pull-right" onclick="javascript:window.location.href='shengxls2.do?method=projectcg'" />
                <%if(keyanuser.getUserclasify().equals("0")){ %>
					<input type="button" value="添加" class="btn btn-success btn-sm pull-right" onclick="javascript:window.location.href='addProjectcg.jsp'"/>
                <%} %>
                          </header>
                          ...
                          <tr>
                             <th><div align="center">学工号</div></th>
                              ...
                              <th class="hidden-phone"><div align="center">项目名称</div></th>
                              <th class="hidden-phone"><div align="center">时间</div></th>
      ...
      for(int i=0;i<list1.size();i++)
      {
       ...
       projectcg=(Projectcg)list1.get(i);
     
      %>
                          <tr class="odd gradeX">
                              <td align="center"><%=projectcg.getPnum()%></td>
                              <td align="center"><%=projectcg.getPName() %></td>
                              <td align="center"><%=projectcg.getPDept() %></td>
                              <td align="center"><%=projectcg.getPTitle() %></td>
                              <td align="center"><%=projectcg.getPTime().toString().split(" ")[0] %></td>
                              <td align="center"><%=projectcg.getPScore() %></td>
                              				...
                              				<!-- 科研人员权限 -->
                              						<%if(keyanuser.getUserclasify().equals("0")){ %>
															...
															<%if(projectcg.getPScore().equals("未审核")){ %>
																  <td align="center">
																  <a  href="projectcg.do?method=detele1&&id=<%=projectcg.getId().toString()%>"onclick="return yesno()"><button class="btn btn-danger btn-xs"><i class="icon-trash"></i>删除</button></a>
																  
																  <a  href="projectcg.do?method=query1&&id=<%=projectcg.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-pencil"></i>修改</button></a>
																  
																   <a  href="projectcg.do?method=queryinfo&&id=<%=projectcg.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
																<%} else{%>
											...
											<!--系科研秘书权限 -->			 
														...
														<td align="center">
															...
															<%if(projectcg.getPScore().equals("未审核")){ %>
																<a href="projectcg.do?method=detele2&&id=<%=projectcg.getId().toString()%>" onclick="return yesno()"><button class="btn btn-danger btn-xs"><i class="icon-trash"></i>删除</button></a>
																
																<a href="projectcg.do?method=update1&&id=<%=projectcg.getId().toString()%>" onclick="return yesno1()"><button class="btn btn-success btn-xs"><i class="icon-ok"></i>审批</button></a>
																<a  href="projectcg.do?method=queryinfo&&id=<%=projectcg.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
																	<%} else{%>
											...
											<!--校科研秘书权限 -->
														...
														<td align="center">
															...
															<%if(projectcg.getPScore().equals("校审通过")){ %>
																<span class="label label-success"><i class="icon-ok">已审核</i></span>
																<a  href="projectcg.do?method=queryinfo&&id=<%=projectcg.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
																<%} else{%>
																<a href="projectcg.do?method=detele3&&id=<%=projectcg.getId().toString()%>" onclick="return yesno()"><button class="btn btn-danger btn-xs"><i class="icon-trash" ></i>删除</button></a>
																<a href="projectcg.do?method=update2&&id=<%=projectcg.getId().toString()%>" onclick="return yesno1()"><button class="btn btn-success btn-xs"><i class="icon-ok" ></i>审批</button></a>
																<a  href="projectcg.do?method=queryinfo&&id=<%=projectcg.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
																	<%}%>
														</td>
														<%} %>
