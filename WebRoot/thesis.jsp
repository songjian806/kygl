...
<html lang="en">
  ...
  <section id="container" class="">
                  ...
                  <div class="col-lg-12">
                      <section class="panel">
                          <header class="panel-heading">
                              	<strong>论文基本信息列表</strong>
                              	<button class="btn btn-default btn-sm pull-right" onclick="javascript:window.location.href='shengxls1.do?method=thesis'"><i class="icon-cloud-download">导出excel</i></button>
                              <%if(keyanuser.getUserclasify().equals("0")){ %>
	                      <button class="btn btn-success btn-sm pull-right" onclick="javascript:window.location.href='addThesis.jsp'"><i class="icon-plus">添加</i></button>
                       <%} %>
  			              </header>
                         ...
                         <thead>
                          <tr>
                              <th class="hidden-phone"><div align="center">学工号</div></th>
                              <th class="hidden-phone"><div align="center">作者</div></th>
                              <th class="hidden-phone"><div align="center">所在单位</div></th>
                              <th class="hidden-phone"><div align="center">论文题目</div></th>
      ...
      {thesis=(Thesis)list1.get(i);
     %>
                          <tr class="odd gradeX">
			                  <td align="center"><%=thesis.getTnum() %></td>
                              <td align="center"><%=thesis.getTName() %></td>
                              <td align="center" class="hidden-phone"><%=thesis.getTUnit() %></td>
                              <td align="center" class="hidden-phone"><%=thesis.getTTitle() %></td>
                              <td align="center" class="hidden-phone"><%=thesis.getTTime().toString().split(" ")[0] %></td>
                              <td align="center" class="hidden-phone"><%=thesis.getTScore() %></td>
                         <!-- 科研人员权限 -->
                           <% if(keyanuser.getUserclasify().equals("0")){ %>
                             <td align="center">
                              <% if(thesis.getTScore().equals("未审核")){ %>
                              
								    <a href="thesis.do?method=detele1&&id=<%=thesis.getId().toString()%>" onclick="return yesno()"><button class="btn btn-danger btn-xs"><i class="icon-trash"></i>删除</button></a>
									<a href="thesis.do?method=query1&&id=<%=thesis.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-pencil"></i>编辑</button></a>
									<a href="thesis.do?method=queryinfo&&id=<%=thesis.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
							  <%} else{%>
							        <button class="btn btn-default btn-xs"><i class="icon-trash" ></i>删除</button>
									<button class="btn btn-default btn-xs"><i class="icon-pencil" ></i>编辑</button>
									<a href="thesis.do?method=queryinfo&&id=<%=thesis.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
						...
						<%if(thesis.getTScore().equals("未审核")){ %>
									<a href="thesis.do?method=detele2&&id=<%=thesis.getId().toString()%>" onclick="return yesno()"><button class="btn btn-danger btn-xs"><i class="icon-trash "></i>删除</button></a>
									<a href="thesis.do?method=update1&&id=<%=thesis.getId().toString()%>" onclick="return yesno1()"><button class="btn btn-success btn-xs"><i class="icon-ok "></i>审批</button></a>
									<a  href="thesis.do?method=queryinfo&&id=<%=thesis.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
						<%} else{%>
						...
						<%} %>
											...
											<!--校科研秘书权限 -->
														...
														<td align="center" class="center hidden-phone">
															...
															<%if(thesis.getTScore().equals("校审通过")){ %>
																<span class="label label-success"><i class="icon-ok">已审核</i></span>
																<a  href="thesis.do?method=queryinfo&&id=<%=thesis.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
																<%} else{%>
																<a href="thesis.do?method=update2&&id=<%=thesis.getId().toString()%>"onclick="return yesno1()"><button class="btn btn-success btn-xs"><i class="icon-ok "></i>审批</button></a>
														
																<a href="thesis.do?method=detele3&&id=<%=thesis.getId().toString()%>" onclick="return yesno()"><button class="btn btn-danger btn-xs"><i class="icon-trash "></i>删除</button></a>
																<a  href="thesis.do?method=queryinfo&&id=<%=thesis.getId().toString()%>"><button class="btn btn-info btn-xs"><i class="icon-eye-open"></i>查看</button></a>
																	<%}%>
														</td>
														<%} %>
